				...
				<div
					...
					<form class="form-horizontal" action="${pageContext.request.contextPath}/user?method=login" method="post">
						...
						<div class="form-group">
							...
							<div class="col-sm-3">
								<input type="text" class="form-control" id="inputPassword3" name="checkcode_session"
									placeholder="验证码">
							</div>
							<div class="col-sm-3">
								<img src="${pageContext.request.contextPath}/checkImg" id="imgId" onclick="changeImg();" style="cursor: pointer;"/>&nbsp;&nbsp;
								<a href="javascript:changeImg();" onclick="changeImg();">换一张</a>
							</div>
						</div>
						<div class="form-group">
						...
						<span  style="color: red ;font-size: 15px;" id="showErrorSpan" >${loginError}</span>
					</form>
				</div>
			</div>
		</div>
...
<script type="text/javascript">
	function changeImg() {
		$("#imgId").attr("src","${pageContext.request.contextPath}/checkImg?t="+new Date().getTime());
	};
	function ajaxInvoke() {
		var uname=$("#username").val();
		var upass=$("#inputPassword").val();
		var ucheck=$("#inputPassword3").val();
		$.ajax({
